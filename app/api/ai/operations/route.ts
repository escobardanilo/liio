...
const requestSchema = z.object({ messages: z.array(messageSchema).min(1).max(30) }).refine((value) => value.messages.at(-1)?.role === "user", { message: "The final message must come from the operator.", path: ["messages"] });
...
export async function POST(request: Request) {
  ...
  try {
    const initial = sanitizeDraft(await generateText({ model: OPERATIONS_MODEL, maxTokens: 900, system: buildOperationsSystemPrompt(interaction.behavior, interaction.operationalRequest), messages }));
    ...
    try {
      ...
      if (!firstEvaluation.pass) {
        const regenerated = sanitizeDraft(await generateText({ model: OPERATIONS_MODEL, maxTokens: 900, system: buildOperationsCorrectionPrompt(interaction.behavior, interaction.operationalRequest, firstEvaluation.reasons), messages }));
